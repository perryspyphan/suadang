...
function ProductModal({
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      try {
        if (isEdit && initial) {
          await updateProductUseCase(initial.id, form)
          onSaved({ ...initial, ...form })
        } else {
          await addProductUseCase({
            ...form, sell_price: Number(form.sell_price), cost_price: Number(form.cost_price),
          ...
          onSaved({ id: '', code: '', name: form.name, created_at: '', supplier_name: null, ...form } as Product)
        }
        onClose()
      } catch (e: any) {
        setErr(e.message)
  ...
  const labelStyle: React.CSSProperties = { fontSize: 11, fontWeight: 600, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 4 }

  return (
    <Overlay>
      <div style={{
        background: '#fff', borderRadius: 16, width: 600, maxHeight: '90vh',
        overflowY: 'auto', padding: 28, boxShadow: '0 8px 40px rgba(0,0,0,0.18)',
      }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px 20px' }}>
          ...
          <div>
            <div style={labelStyle}>Loại hàng</div>
          ...
          <div>
            ...
            <input style={inp} type="number" value={form.sell_price} onChange={e => set('sell_price', Number(e.target.value))} min={0} />
          </div>

          <div>
            <div style={labelStyle}>Giá vốn (₫)</div>
            <input style={inp} type="number" value={form.cost_price} onChange={e => set('cost_price', Number(e.target.value))} min={0} />
          </div>

          <div>
            <div style={labelStyle}>Tồn kho hiện tại</div>
            <input style={inp} type="number" value={form.stock} onChange={e => set('stock', Number(e.target.value))} min={0} />
          </div>

          <div>
            <div style={labelStyle}>Dự kiến đặt hàng</div>
            <input style={inp} type="number" value={form.expected_order} onChange={e => set('expected_order', Number(e.target.value))} min={0} />
          </div>

          <div>
            <div style={labelStyle}>Định mức tồn Min</div>
            <input style={inp} type="number" value={form.min_stock} onChange={e => set('min_stock', Number(e.target.value))} min={0} />
          </div>

          <div>
            <div style={labelStyle}>Định mức tồn Max</div>
            <input style={inp} type="number" value={form.max_stock} onChange={e => set('max_stock', Number(e.target.value))} min={0} />
          </div>

          <div>
            <div style={labelStyle}>Thương hiệu</div>
            <input style={inp} value={form.brand} onChange={e => set('brand', e.target.value)} placeholder="Chưa có" />
          </div>

          <div>
            <div style={labelStyle}>Vị trí kho</div>
            <input style={inp} value={form.location} onChange={e => set('location', e.target.value)} placeholder="Aisle 4, Shelf B" />
          </div>

          <div style={{ gridColumn: '1/-1' }}>
            <div style={labelStyle}>Nhà cung cấp (ID)</div>
            <input style={inp} value={form.supplier_id} onChange={e => set('supplier_id', e.target.value)} placeholder="Mã nhà cung cấp" />
          </div>

          <div style={{ gridColumn: '1/-1' }}>
            <div style={labelStyle}>URL Hình ảnh</div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 20 }}>
          <button onClick={onClose} style={{ border: '1px solid #e2e8f0', background: '#fff', borderRadius: 8, height: 36, padding: '0 20px', fontSize: 13, cursor: 'pointer', color: '#49636F' }}>
            Hủy
          </button>
          <button
            onClick={handleSubmit}
            disabled={isPending}
            style={{ ...btnGreen, borderRadius: 8, height: 36, padding: '0 28px' }}
          >
...
function ExpandedRow({ product, onEdit }: { product: Product; onEdit: () => void }) {
  ...
  const labelStyle: React.CSSProperties = {
    ...
    textTransform: 'uppercase', letterSpacing: '0.5px', lineHeight: '20px',
  }
  const valueStyle: React.CSSProperties = { fontSize: 14, fontWeight: 500, color: '#2A3437', lineHeight: '20px' }

  function Field({ label, value, color }: { label: string; value: React.ReactNode; color?: string }) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        <div style={labelStyle}>{label}</div>
        <div style={{ ...valueStyle, color: color ?? valueStyle.color }}>{value ?? <span style={{ color: '#94A3B8' }}>Chưa có</span>}</div>
      </div>
    )
  }
  ...
  return (
    <div style={{
      background: '#fff', borderLeft: '4px solid rgba(0,110,28,0.2)', borderRight: '4px solid rgba(0,110,28,0.2)',
      borderRadius: 8, padding: '9px 0', boxShadow: 'inset 0 2px 10px rgba(0,0,0,0.02)',
    }}>
      {/* Tabs */}
      <div style={{ display: 'flex', gap: 4, padding: '16px 32px 0' }}>
        {(['info', 'stock'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} style={{
            padding: '10px 24px', borderRadius: '4px 4px 0 0', border: tab === t ? '1px solid #F1F5F9' : 'none',
            borderBottom: tab === t ? '1px solid #fff' : 'none', background: '#fff',
            fontWeight: tab === t ? 600 : 500, fontSize: 12, cursor: 'pointer',
      ...
      <div style={{ borderTop: '1px solid #F1F5F9', padding: '32px', display: 'flex', gap: 32 }}>
        {/* Image column */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24, width: 256, flexShrink: 0 }}>
          <div style={{
            width: 256, height: 256, background: '#E1EAEC', border: '1px solid #F1F5F9',
            ...
            <div style={{
              ...
              background: 'rgba(255,255,255,0.9)', boxShadow: '0 1px 2px rgba(0,0,0,0.05)',
              backdropFilter: 'blur(4px)', borderRadius: 4, padding: '4px 8px',
              fontSize: 10, fontWeight: 600, color: '#006E1C',
            }}>Ảnh chính</div>
          </div>
          ...
          <div style={{ background: '#F8FAFC', borderRadius: 8, padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ width: 14, height: 14, borderRadius: '50%', background: product.can_sell_direct ? '#16A34A' : '#94A3B8', flexShrink: 0 }} />
              <span style={{ fontSize: 12, fontWeight: 500, color: product.can_sell_direct ? '#2A3437' : '#94A3B8' }}>
                {product.can_sell_direct ? 'Bán trực tiếp' : 'Không bán trực tiếp'}
              </span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ width: 14, height: 14, borderRadius: '50%', background: product.has_points ? '#16A34A' : '#9E422C', flexShrink: 0 }} />
              <span style={{ fontSize: 12, fontWeight: product.has_points ? 500 : 400, color: product.has_points ? '#2A3437' : '#566164' }}>
        ...
        {/* Data column */}
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: 'Manrope, Inter, sans-serif', fontWeight: 700, fontSize: 24, color: '#0F172A', marginBottom: 24 }}>
            {product.name}
          </div>
          ...
          {tab === 'info' && (
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '24px 0', marginBottom: 24 }}>
                <Field label="Mã hàng" value={<span style={{ fontFamily: 'monospace', color: '#2A3437' }}>{product.code}</span>} />
                <Field label="Nhóm hàng" value={
                  <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: groupColors[product.group] ?? '#94A3B8', display: 'inline-block' }} />
                    {product.group}
                  </span>
                } />
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '24px 0', marginBottom: 24 }}>
                ...
                <Field label="Định mức tồn" value={`Min: ${product.min_stock} | Max: ${product.max_stock}`} />
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginTop: 8 }}>
                <div>
                  <div style={{ fontSize: 10, fontWeight: 600, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 8 }}>
                    Ghi chú đặt hàng
                  </div>
                  <div style={{
                    background: '#F8FAFC', borderRadius: 8, padding: 12, minHeight: 72, fontSize: 14,
                    color: product.note ? '#2A3437' : '#6B7280',
                  }}>
                    {product.note || 'Ghi chú cho món hàng...'}
                  </div>
                </div>
                <div>
                  <div style={{ fontSize: 10, fontWeight: 600, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 8 }}>
                  ...
                  <div style={{
                    border: '1px solid #F1F5F9', borderRadius: 8, padding: 12, fontSize: 14,
                    color: '#2A3437', display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                  }}>
                    <span>{product.supplier_name ?? product.supplier_id ?? <span style={{ color: '#94A3B8' }}>Chưa có</span>}</span>
                    <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                      <path d="M1 5h8M5 1l4 4-4 4" stroke="#CBD5E1" strokeWidth="1.3" strokeLinecap="round" />
                    </svg>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 8 }}>
                    <button style={{ border: 'none', background: 'none', borderRadius: 8, height: 36, padding: '0 20px', fontSize: 14, cursor: 'pointer', color: '#49636F', fontWeight: 600 }}>
                      Hủy
                    </button>
                    <button
                      onClick={onEdit}
                      style={{ background: '#006E1C', color: '#fff', border: 'none', borderRadius: 8, height: 36, padding: '0 28px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}
                    >
                      Cập nhật
                    </button>
                  </div>
          ...
          {tab === 'stock' && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 20 }}>
              <div style={{ padding: 16, background: '#f0fdf4', borderRadius: 12, textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 28, fontWeight: 700, color: stockColor(product) }}>{product.stock}</div>
              </div>
              <div style={{ padding: 16, background: '#f8fafc', borderRadius: 12, textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 28, fontWeight: 700, color: '#2A3437' }}>{product.min_stock}</div>
              </div>
              <div style={{ padding: 16, background: '#f8fafc', borderRadius: 12, textAlign: 'center' }}>
                <div style={{ fontSize: 11, color: '#94A3B8', fontWeight: 600, textTransform: 'uppercase', marginBottom: 8 }}>Định mức Max</div>
                <div style={{ fontSize: 28, fontWeight: 700, color: '#2A3437' }}>{product.max_stock}</div>
              </div>
              <div style={{ padding: 16, background: '#f8fafc', borderRadius: 12, textAlign: 'center' }}>
                <div style={{ fontSize: 11, color: '#94A3B8', fontWeight: 600, textTransform: 'uppercase', marginBottom: 8 }}>Dự kiến đặt hàng</div>
                <div style={{ fontSize: 28, fontWeight: 700, color: '#2563EB' }}>{product.expected_order ?? '—'}</div>
              </div>
              <div style={{ padding: 16, background: '#f8fafc', borderRadius: 12, textAlign: 'center' }}>
                <div style={{ fontSize: 11, color: '#94A3B8', fontWeight: 600, textTransform: 'uppercase', marginBottom: 8 }}>Vị trí</div>
                <div style={{ fontSize: 16, fontWeight: 600, color: '#2A3437', marginTop: 10 }}>{product.location ?? '—'}</div>
              </div>
            </div>
          )}
        </div>
...
// ── Main Table Row ────────────────────────────────────────────
function TableRow({
  product, selected, expanded,
  onSelect, onExpand, onEdit,
}: {
  product: Product
  selected: boolean
  expanded: boolean
  ...
  return (
    <>
      <div onClick={onExpand} style={{
        ...
        borderTop: '1px solid #F3F4F6', background: '#fff', cursor: 'pointer',
      ...
      >
        ...
        {/* Stock */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', padding: '18px 12px' }}>
          <span style={{ fontSize: 14, fontWeight: 700, color: stockColor(product) }}>{product.stock}</span>
        </div>
        {/* Expected order */}
        <div style={{ display: 'flex', alignItems: 'center', padding: '18px 12px' }}>
          {product.expected_order
            ? <span style={{ fontSize: 12, color: '#EF4444', fontWeight: 400 }}>+{product.expected_order}</span>
      ...
      {/* Expanded Detail */}
      {expanded && (
        <div style={{ padding: '0 8px 0', background: '#fff' }}>
          <ExpandedRow product={product} onEdit={onEdit} />
        </div>
      )}
    </>
  )
...
function Sidebar({
  ...
  const sHead: React.CSSProperties = { fontWeight: 600, fontSize: 14, color: '#333', marginBottom: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {TYPES.map(t => (
            <label key={t} style={sLabel}>
              <input type="checkbox" checked={typeFilter === t} onChange={() => setTypeFilter(typeFilter === t ? '' : t)}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {GROUPS.map(g => (
            <div key={g}
              ...
              style={{
                padding: '6px 8px', borderRadius: 4, cursor: 'pointer', fontSize: 14, color: '#333',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {[
            ...
            <label key={opt.val} style={sLabel}>
              <input type="radio" name="stock_filter" value={opt.val} checked={stockFilter === opt.val}
...
export default function ProductTable({ initialProducts }: { initialProducts: Product[] }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 16, padding: 16, minHeight: '100vh', background: '#f9fafb' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {[
            ...
            { label: 'Dự kiến hết hàng', align: 'left' },
          ].map(col => (
            <div key={col.label} style={{ padding: 12, fontSize: 14, fontWeight: 600, color: '#374151', textAlign: col.align as any }}>
              {col.label}
            </div>
          ))}
        </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 12, color: '#6B7280' }}>
              Đang xem {Math.min((page - 1) * PER + 1, filtered.length)}–{Math.min(page * PER, filtered.length)} / {filtered.length} hàng hóa
            </span>
            <select style={{ height: 30, border: '1px solid #E5E7EB', borderRadius: 4, fontSize: 14, color: '#6B7280', padding: '0 4px' }}>
      ...
      {showConfirmDelete && (
        <ConfirmDeleteModal count={selected.size} isPending={isPending} onConfirm={handleDelete} onCancel={() => setShowConfirmDelete(false)} />
      )}
