...
const roleBadge = (r: string) => {
  ...
  const label = r === 'admin' ? 'Admin' : 'Nhân viên'
...
export default function AccountTable({ initialAccounts, employees }: Props) {
  ...
  const doAdd = () => {
    ...
    startTransition(async () => {
      await addAccountUseCase({ name: fName, email: fEmail, role: fRole, employee_id: fEmployeeId })
...
function AccountForm({ title, employees, name, email, role, employeeId, setName, setEmail, setRole, setEmployeeId, onSave, onCancel, isPending }: {
  ...
  name: string; email: string; role: 'admin' | 'staff'; employeeId: string
  setName: (v: string) => void; setEmail: (v: string) => void
  setRole: (v: 'admin' | 'staff') => void; setEmployeeId: (v: string) => void
    ...
    <div style={{ background: '#fff', borderRadius: 10, padding: '20px 18px', width: 320, boxShadow: '0 8px 32px rgba(0,0,0,0.2)' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <div>
          ...
          <select value={role} onChange={e => setRole(e.target.value as 'admin' | 'staff')} style={{ ...inp, appearance: 'none', cursor: 'pointer' }}>
